  ...
  return (
    <section className="min-h-screen bg-white dark:bg-darkbg1 py-8 sm:py-10 px-3 sm:px-6 md:px-10 transition-colors duration-300">
      <motion.h1
        ...
        className="text-xl sm:text-3xl md:text-4xl font-bold text-center text-navy dark:text-white mb-6 sm:mb-12"
      >
        Our SaaS Products
      </motion.h1>
      ...
      {products.map((product, index) => (
        <motion.section
          ...
          } items-center gap-4 sm:gap-10 py-8 sm:py-16 border-t border-gray-200 dark:border-gray-700`}
        >
          <motion.div
            whileHover={{ scale: 1.02 }}
            transition={{ type: "spring", stiffness: 200 }}
            className="w-full md:w-1/2 h-40 sm:h-64 md:h-96 relative rounded-xl sm:rounded-2xl overflow-hidden shadow-md"
          >
            <Image
              src={product.image}
              alt={product.name}
          ...
          <div className="w-full md:w-1/2">
            <h3 className="text-lg sm:text-3xl font-semibold text-navy dark:text-white mb-1 sm:mb-2">
              {product.name}
            </h3>
            <p className="text-[11px] sm:text-sm text-gray-500 dark:text-gray-400 mb-2 sm:mb-4 uppercase tracking-wide">
              {product.category}
            </p>
            <p className="text-sm sm:text-lg text-gray-700 dark:text-gray-300 leading-relaxed mb-4 sm:mb-6">
              {product.description}
            </p>
