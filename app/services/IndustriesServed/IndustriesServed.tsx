        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-6">
          {industries.map(({ name, icon: Icon }, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="py-4 sm:py-6 px-2 sm:px-4 border border-gray-200 dark:border-gray-700 rounded bg-gray-50 dark:bg-darkbg2 text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 hover:shadow-md hover:border-gold transition-all duration-300 flex flex-col items-center space-y-2 sm:space-y-3"
            >
              <Icon size={24} className="sm:size-[32px] text-gold" />
              <span>{name}</span>
            </motion.div>
          ))}
        </div>
